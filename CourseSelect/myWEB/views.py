...
from .models import CalculatorTable, StudentTable, TeacherTable, ClassTable, ScoreTable, AssessTable
...
def register(request):
    if request.method == 'POST':
        ...
        if not User.objects.filter(username=username).exists():
            ...
            return render(request, 'register.html', {'register_obj': '用户已经存在'})
        else:
    ...
    if request.method == 'GET':  # 首次访问
        ...
        for i in range(1, 5):
            ...
            if len(kh) and len(gh):
                ...
                for bl in result:
                    if bl.syrl == 0:
                        context['result' + str(i)] = '选课失败：该课程余量为0, 已进入待选课程'
                        classview = []
                        classview.append(
                            {
                                'kh': bl.kh, 'gh': bl.gh_id,
                                'km': bl.km, 'xm': bl.gh.xm,
                                'syrl': bl.syrl
                            }
                        )
                        context['classview'] = classview
                        f = False
                    else:
                        bl.syrl = bl.syrl - 1
                        bl.save()
...
@login_required
def pingjia(request):  # 学生评价
    status, context = get_user_info(request)
    if status != 'student':
        return HttpResponseRedirect("/")
    if request.method == 'GET':  # 首次访问
        return render(request, 'pingjia.html', context=context)
    elif request.method == 'POST':  # 填表评教
        for i in range(1, 2):
            kh = context['kh' + str(i)] = request.POST['kh' + str(i)]
            gh = context['gh' + str(i)] = request.POST['gh' + str(i)]
            if len(kh) and len(gh):
                result = ClassTable.objects.filter(xq='2019-2020学年冬季学期', kh=kh, gh=gh)
                if not result.exists():
                    context['result' + str(i)] = '评教失败：不存在该课程'
                    continue
                for kc in result:
                    r = AssessTable.objects.filter(cid=kc.id)
                classtable = []
                for kc in r:
                    classtable.append(kc.content)
            elif len(kh) or len(gh):
                context['result' + str(i)] = '评教失败：课程号或工号未填写完整'
            context['classtable'] = classtable
        return render(request, 'his_ass.html', context=context)
    return HttpResponseRedirect("/")


def wode_pingjia(request):  # 学生评价
    status, context = get_user_info(request)
    if status != 'student':
        return HttpResponseRedirect("/")
    if request.method == 'POST':  # 填表评教
            kh = context['kh'] = request.POST['kh']
            gh = context['gh'] = request.POST['gh']
            nr = context['nr'] = request.POST['nr']
            if len(kh) and len(gh) and len(nr):
                result = ClassTable.objects.filter(xq='2019-2020学年冬季学期', kh=kh, gh=gh)
                for kc in result:
                    key = ClassTable.objects.get(id=kc.id)
                    item = AssessTable(id='4', content=nr, cid=key)
                item.save()
